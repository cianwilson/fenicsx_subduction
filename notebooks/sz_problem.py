...
# We can pass these into the class (which calls the `__init__` function) and it will generate our meshes (hence we will get a lot of output again), initialize our functionspaces and `Function`s, and setup the boundary conditions.
...
# Because we have added functions to the class we need to re-instantiate it, which unfortunately means more output during mesh (re-)generation.
